...
export function App({ email, signOut }: { email: string; signOut: (() => void) | undefined }) {
  ...
  return <div className="app-shell">
    <aside className="app-sidebar">
      <div className="brand"><span className="brand-mark">S</span><div><strong>SIGNIFY</strong><small>Control plane</small></div></div>
      <nav aria-label="主要導覽">
        <button className={view === "operations" ? "active" : ""} onClick={() => setView("operations")}><span>⌁</span><div>裝置控制<small>即時狀態與指令</small></div></button>
        <button className={view === "configuration" ? "active" : ""} onClick={() => setView("configuration")}><span>◫</span><div>課程設定<small>草稿、發布與版本</small></div></button>
        <button className={view === "data" ? "active" : ""} onClick={() => setView("data")}><span>⇩</span><div>資料匯出<small>研究資料與保存</small></div></button>
      </nav>
    ...
    <main className="app-main">
      {view === "operations" && <>
        <section className="page-heading operations-heading"><div><span className="eyebrow">LIVE OPERATIONS</span><h1>裝置控制台</h1><p>以裝置回報為真實狀態；操作意圖與執行結果分開顯示。</p></div><div className="heading-actions"><button className="secondary" onClick={() => void refresh()}>↻ 重新整理</button><button onClick={() => openPairing()}>＋ 配對裝置</button></div></section>
        <section className="guardrail"><span>i</span><div><strong>Unity 保有 transition authority</strong><p>Accepted／Deferred 只代表指令進入 Unity，不代表 section 已 Activated。請以右側裝置快照為準。</p></div></section>
        <section className="metric-row"><article><span className="metric-icon green">●</span><div><small>Ready devices</small><strong>{onlineCount}</strong></div></article><article><span className="metric-icon blue">◇</span><div><small>Registered</small><strong>{devices.length}</strong></div></article><article><span className="metric-icon amber">→</span><div><small>Command state</small><strong className="metric-text">{trackedCommand ? commandLabel(trackedCommand.status) : "No active command"}</strong></div></article></section>
        <div className="operations-grid">
          <section className="surface device-panel"><div className="panel-heading"><div><span className="eyebrow">DEVICE REGISTRY</span><h2>裝置</h2></div><button className="icon-button" aria-label="新增配對" onClick={() => openPairing()}>＋</button></div>
            {devices.length ? <div className="device-list">{devices.map((item) => <button key={item.deviceId} className={`device-row ${selected === item.deviceId ? "selected" : ""}`} onClick={() => { setSelected(item.deviceId); setDesired(null); }}><span className={`device-status ${item.ready ? "online" : ""}`}/><div><strong>{item.deviceId}</strong><small>{item.ready ? "Ready · snapshot synced" : item.revokedAt ? "Credential revoked" : "Offline / syncing"}</small></div><span>›</span></button>)}</div> : <div className="empty-state"><span>＋</span><strong>尚未配對裝置</strong><p>建立五分鐘有效的一次性配對碼。</p><button onClick={() => openPairing()}>配對第一台裝置</button></div>}
          </section>
          <div className="device-workspace">
            {!device ? <section className="surface empty-device"><span>⌁</span><h2>選擇一台裝置</h2><p>從左側選取裝置，或先建立新的配對。</p></section> : <>
              <section className="device-title"><div><span className={`device-status large ${device.ready ? "online" : ""}`}/><div><span className="eyebrow">SELECTED DEVICE</span><h2>{device.deviceId}</h2><p>{device.ready ? `最後同步 ${relativeTime(device.lastSeenAt)}` : "裝置目前離線或尚未完成同步"}</p></div></div><div className="heading-actions"><button className="secondary" onClick={() => openPairing(device.deviceId)}>重新配對</button><button className="text-danger" onClick={() => setRevokeConfirm(true)}>撤銷 credential</button></div></section>
              <div className="truth-intent-grid">
                <section className="surface truth-card"><div className="card-kicker"><span className="truth-mark">✓</span>DEVICE-REPORTED TRUTH</div><strong className="section-name">{actual ?? "尚無快照"}</strong><p>{device.snapshot?.currentSection?.sectionInstanceId ?? "等待 authoritative snapshot"}</p><dl><div><dt>Device state revision</dt><dd>{device.snapshot?.deviceStateRevision ?? "—"}</dd></div><div><dt>Config revision</dt><dd>{device.snapshot?.configRevision ?? "—"}</dd></div><div><dt>Transition</dt><dd>{device.snapshot?.transition?.state ?? "idle / unknown"}</dd></div><div><dt>Boot plan</dt><dd>{device.snapshot?.bootPlanState ?? "—"}</dd></div></dl></section>
                <section className="surface intent-card"><div className="card-kicker"><span className="intent-mark">→</span>OPERATOR INTENT</div><h3>切換課程 section</h3><div className="section-picker">{sections.map(([id, label]) => <button key={id} className={`${desired === id ? "selected" : ""} ${actual === id ? "actual" : ""}`} disabled={!device.ready} onClick={() => setDesired(id)}><span>{label}</span>{actual === id && <small>目前</small>}</button>)}</div><div className="intent-footer"><p>{desired && desired !== actual ? <>準備送往 <strong>{desired}</strong>；目前仍是 <strong>{actual ?? "未知"}</strong></> : "選擇目標 section 後才會啟用送出。"}</p><button disabled={!device.ready || !desired || desired === actual} onClick={() => void navigate()}>送出切換意圖</button></div></section>
              </div>
              {trackedCommand && <section className={`command-tracker surface status-${trackedCommand.status}`}><div><span className="pulse-dot"/><div><span className="eyebrow">COMMAND LIFECYCLE</span><strong>{commandLabel(trackedCommand.status)}</strong><small>{trackedCommand.commandType ?? "command"} · {shortId(trackedCommand.commandId)}</small></div></div><div className="lifecycle"><span className="done">Queued</span><i/><span className={statusReached(trackedCommand.status, "accepted") ? "done" : ""}>ACK</span><i/><span className={terminalStatuses.includes(trackedCommand.status) ? "done" : ""}>Terminal</span></div></section>}
              <div className="control-grid"><section className="surface control-card"><div><span className="control-icon">↻</span><h3>同步與階段</h3><p>要求完整快照，或在 LearnWord 穩定階段前進。</p></div><div className="stack-actions"><button className="secondary" disabled={!device.ready} onClick={() => void send("request_snapshot", {})}>要求完整快照</button><button disabled={!device.snapshot?.learnWord || device.snapshot.learnWord.phaseTransitioning} onClick={() => void send("advance_subsection", { expectedPhase: device.snapshot!.learnWord!.phase, expectedPhaseEpoch: device.snapshot!.learnWord!.phaseEpoch })}>LearnWord 下一階段</button></div></section>
                <section className="surface control-card"><div><span className="control-icon">◎</span><h3>人工判定</h3><p>{device.snapshot?.activeJudgment ? `Attempt ${shortId(device.snapshot.activeJudgment.attemptId)}` : "目前沒有 active attempt；按鈕保持停用。"}</p></div><div className="judgment-actions"><button disabled={!device.snapshot?.activeJudgment} onClick={() => void send("force_judgment", { outcome: "correct" })}>✓ Correct</button><button className="danger" disabled={!device.snapshot?.activeJudgment} onClick={() => void send("force_judgment", { outcome: "wrong" })}>× Wrong</button></div></section></div>
            </>}
          </div>
        </div>
      </>}
      ...
      {view === "data" && <div className="data-workspace"><section className="page-heading"><div><span className="eyebrow">RESEARCH DATA</span><h1>研究資料匯出</h1><p>產生已遮蔽敏感資訊的短效下載連結。</p></div></section><section className="guardrail"><span>i</span><div><strong>下載連結有效 15 分鐘</strong><p>匯出檔保留 7 天；內容不包含 device credential、pairing code 或 operator email。</p></div></section><div className="export-grid"><article className="surface export-card"><span className="export-icon">{`{ }`}</span><h2>JSON</h2><p>保留巢狀 snapshot、command、ACK 與 terminal result，適合程式分析。</p><button disabled={Boolean(exportBusy)} onClick={() => void createExport("json")}>{exportBusy === "json" ? "產生中…" : "產生 JSON 匯出"}</button></article><article className="surface export-card"><span className="export-icon">▦</span><h2>CSV</h2><p>扁平化主要欄位，適合試算表檢視與快速統計。</p><button disabled={Boolean(exportBusy)} onClick={() => void createExport("csv")}>{exportBusy === "csv" ? "產生中…" : "產生 CSV 匯出"}</button></article></div>{exportResult && <section className="surface download-ready"><span className="modal-icon success">✓</span><div><h2>匯出已準備完成</h2><p>{exportResult.itemCount} 筆資料 · {exportResult.format.toUpperCase()} · 連結將於 {Math.round(exportResult.downloadExpiresInSeconds / 60)} 分鐘後失效</p></div><a className="button-link" href={exportResult.downloadUrl} target="_blank" rel="noreferrer">下載檔案 ↗</a></section>}</div>}
    </main>
    ...
    {revokeConfirm && <ConfirmModal
      icon="!" title="撤銷裝置 credential？"
      description={`${device?.deviceId ?? "此裝置"} 會立即失去 REST 與新 WebSocket 連線權限，之後需要重新配對。`}
      danger confirmLabel="撤銷 credential" busy={modalBusy} onCancel={() => setRevokeConfirm(false)} onConfirm={() => void revokeDevice()}
    />}
  </div>;
}
